...
public class InventoryClick implements Listener {
    ...
    public void onSaddleEquipFromInventory(InventoryClickEvent e) {
        switch (e.getAction()) {
            ...
            case PLACE_ONE:
            case PLACE_SOME:
                if (e.getCursor() != null && e.getCursor().getType() == Material.SADDLE && e.getInventory().getHolder() instanceof AbstractHorse && horseHandler.containsHorse(e.getCursor())) {
            ...
            case MOVE_TO_OTHER_INVENTORY:
                if (e.getCurrentItem() != null) {
                    if (e.getCurrentItem().getType() != Material.AIR && e.getInventory().getHolder() instanceof AbstractHorse &&
                            horseHandler.containsHorse(e.getCurrentItem())) {
                        e.getWhoClicked().sendMessage(Localization.getPrefix() + ChatColor.RED +Localization.getMessage((Player) e.getWhoClicked(), "saddleInUse"));
                        e.setCancelled(true);
                    }
                    break;
